...
const std::string SqlCase::TypeString(hybridse::type::Type type) { return node::TypeName(type); }
...
static bool ParseSqlCaseNode(const YAML::Node& sql_case_node, const YAML::Node& expect_provider,
    ...
    *is_skip = false;
    ...
    if (sql_case_node["parameters"]) {
        if (!SqlCase::CreateTableInfoFromYamlNode(sql_case_node["parameters"], &sql_case.parameters_)) {
            return false;
        }
    }
...
std::string FindSqlCaseBaseDirPath() { return SqlCase::SqlCaseBaseDir(); }
