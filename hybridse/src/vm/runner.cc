...
ClusterTask RunnerBuilder::Build(PhysicalOpNode* node, Status& status) {
    ...
    switch (node->GetOpType()) {
        ...
        default: {
            status.code = common::kExecutionPlanError;
            status.msg =
                "Non-support node " + PhysicalOpTypeName(node->GetOpType()) + " for OpenMLDB Online execute mode";
            LOG(WARNING) << status;
            return RegisterTask(node, fail);
        }
    }
...
Row Runner::WindowProject(const int8_t* fn, const uint64_t row_key, const Row row, const codec::Row& parameter,
                          const bool is_instance, size_t append_slices, Window* window) {
    if (row.empty()) {
        return row;
    }
    ...
    auto udf = reinterpret_cast<int32_t (*)(const int64_t key, const int8_t*, const int8_t*, const int8_t*, int8_t**)>(
...
std::shared_ptr<DataHandler> RowProjectRunner::Run(RunnerContext& ctx,
    ...
    return std::shared_ptr<RowHandler>(new MemRowHandler(project_gen_.Gen(row->GetValue(), ctx.GetParameterRow())));
}
...
std::shared_ptr<DataHandler> SimpleProjectRunner::Run(RunnerContext& ctx,
    ...
    switch (input->GetHanlderType()) {
        case kTableHandler: {
            return std::shared_ptr<TableHandler>(
                new TableProjectWrapper(std::dynamic_pointer_cast<TableHandler>(input), parameter, &project_gen_.fun_));
        }
        case kPartitionHandler: {
            return std::shared_ptr<TableHandler>(new PartitionProjectWrapper(
                std::dynamic_pointer_cast<PartitionHandler>(input), parameter, &project_gen_.fun_));
        }
        case kRowHandler: {
            return std::shared_ptr<RowHandler>(
                new RowProjectWrapper(std::dynamic_pointer_cast<RowHandler>(input), parameter, &project_gen_.fun_));
        }
        default: {
...
std::shared_ptr<DataHandler> SelectSliceRunner::Run(RunnerContext& ctx,
    ...
    switch (input->GetHanlderType()) {
        case kTableHandler: {
            return std::shared_ptr<TableHandler>(
                new TableProjectWrapper(std::dynamic_pointer_cast<TableHandler>(input), parameter, &get_slice_fn_));
        }
        case kPartitionHandler: {
            return std::shared_ptr<TableHandler>(new PartitionProjectWrapper(
                std::dynamic_pointer_cast<PartitionHandler>(input), parameter, &get_slice_fn_));
        }
        case kRowHandler: {
            return std::make_shared<RowProjectWrapper>(std::dynamic_pointer_cast<RowHandler>(input), parameter,
                                                       &get_slice_fn_);
        }
        default: {
...
std::shared_ptr<DataHandler> WindowAggRunner::Run(RunnerContext& ctx,
    ...
    auto instance_partition = instance_window_gen_.partition_gen_.Partition(input, parameter);
    ...
    while (instance_partition_iter->Valid()) {
        ...
        RunWindowAggOnKey(parameter, instance_partition, union_partitions, join_right_tables, key, output_table);
...
void WindowAggRunner::RunWindowAggOnKey(const Row& parameter, std::shared_ptr<PartitionHandler> instance_partition,
    ...
    while (instance_segment_iter->Valid()) {
        ...
        while (min_union_pos >= 0 && union_segment_status[min_union_pos].key_ < instance_order) {
            ...
            window_project_gen_.Gen(union_segment_iters[min_union_pos]->GetKey(), row, parameter, false, append_slices_,
                                    &window);
        ...
        if (windows_join_gen_.Valid()) {
            ...
            output_table->AddRow(window_project_gen_.Gen(instance_segment_iter->GetKey(), instance_row, parameter, true,
                                                         append_slices_, &window));
        }
    ...
    switch (left->GetHanlderType()) {
        case kTableHandler: {
            ...
            if (kPartitionHandler == right->GetHanlderType()) {
                if (!join_gen_.TableJoin(left_table, std::dynamic_pointer_cast<PartitionHandler>(right), parameter,
                                         output_table)) {
                    return fail_ptr;
                }
            } else {
                if (!join_gen_.TableJoin(left_table, std::dynamic_pointer_cast<TableHandler>(right), parameter,
                                         output_table)) {
                    return fail_ptr;
                }
            }
        ...
        case kPartitionHandler: {
            ...
            if (kPartitionHandler == right->GetHanlderType()) {
                if (!join_gen_.PartitionJoin(left_partition, std::dynamic_pointer_cast<PartitionHandler>(right),
                                             parameter, output_partition)) {
                    return fail_ptr;
                }

            } else {
                if (!join_gen_.PartitionJoin(left_partition, std::dynamic_pointer_cast<TableHandler>(right), parameter,
                                             output_partition)) {
                    return fail_ptr;
                }
            }
        ...
        case kRowHandler: {
            ...
            return std::make_shared<MemRowHandler>(join_gen_.RowLastJoin(left_row->GetValue(), right, parameter));
        }
        default:
            return fail_ptr;
    }
...
std::shared_ptr<PartitionHandler> PartitionGenerator::Partition(std::shared_ptr<DataHandler> input,
    ...
    switch (input->GetHanlderType()) {
        case kPartitionHandler: {
            return Partition(std::dynamic_pointer_cast<PartitionHandler>(input), parameter);
        }
        case kTableHandler: {
...
Row JoinGenerator::RowLastJoin(const Row& left_row, std::shared_ptr<DataHandler> right, const Row& parameter) {
    switch (right->GetHanlderType()) {
        case kPartitionHandler: {
            return RowLastJoinPartition(left_row, std::dynamic_pointer_cast<PartitionHandler>(right), parameter);
        }
        case kTableHandler: {
            return RowLastJoinTable(left_row, std::dynamic_pointer_cast<TableHandler>(right), parameter);
        }
        case kRowHandler: {
...
Row JoinGenerator::RowLastJoinTable(const Row& left_row, std::shared_ptr<TableHandler> table, const Row& parameter) {
...
bool JoinGenerator::TableJoin(std::shared_ptr<TableHandler> left, std::shared_ptr<TableHandler> right,
                              const Row& parameter, std::shared_ptr<MemTimeTableHandler> output) {
...
bool JoinGenerator::TableJoin(std::shared_ptr<TableHandler> left, std::shared_ptr<PartitionHandler> right,
    ...
    while (left_iter->Valid()) {
        const Row& left_row = left_iter->GetValue();
        std::string key_str = index_key_gen_.Valid() ? index_key_gen_.Gen(left_row, parameter) : "";
        if (left_key_gen_.Valid()) {
...
bool JoinGenerator::PartitionJoin(std::shared_ptr<PartitionHandler> left, std::shared_ptr<TableHandler> right,
                                  const Row& parameter, std::shared_ptr<MemPartitionHandler> output) {
    ...
    while (left_window_iter->Valid()) {
        ...
        while (left_iter->Valid()) {
            ...
            output->AddRow(key_str, left_iter->GetKey(),
                           Runner::RowLastJoinTable(left_slices_, left_row, right_slices_, right, parameter,
    ...
    while (left_partition_iter->Valid()) {
        ...
        while (left_iter->Valid()) {
            ...
            output->AddRow(left_key_str, left_iter->GetKey(),
                           Runner::RowLastJoinTable(left_slices_, left_row, right_slices_, right_table, parameter,
                                                    right_sort_gen_, condition_gen_));
            left_iter->Next();
        }
        left_partition_iter->Next();
...
const Row Runner::RowLastJoinTable(size_t left_slices, const Row& left_row, size_t right_slices,
                                   std::shared_ptr<TableHandler> right_table, const Row& parameter,
                                   SortGenerator& right_sort, ConditionGenerator& cond_gen) {
...
std::shared_ptr<DataHandler> ProxyRequestRunner::RunWithRowInput(RunnerContext& ctx,  // NOLINT
    ...
    if (!tablet) {
        ...
        if (ctx.sp_name().empty()) {
            return tablet->SubQuery(task_id_, cluster_job->db(), cluster_job->sql(), row, false, ctx.is_debug());
        } else {
            return tablet->SubQuery(task_id_, cluster_job->db(), ctx.sp_name(), row, true, ctx.is_debug());
        }
    }
}
// out_table = Proxy(in_table) , remote table left join
std::shared_ptr<TableHandler> ProxyRequestRunner::RunWithRowsInput(RunnerContext& ctx,  // NOLINT
    ...
    if (ctx.sp_name().empty()) {
        return tablet->SubQuery(task_id_, cluster_job->db(), cluster_job->sql(),
                                ctx.cluster_job()->common_column_indices(), rows, request_is_common, false,
                                ctx.is_debug());
    } else {
        return tablet->SubQuery(task_id_, cluster_job->db(), ctx.sp_name(), ctx.cluster_job()->common_column_indices(),
...
const bool ConditionGenerator::Gen(std::shared_ptr<TableHandler> table, const codec::Row& parameter) {
    ...
    return Runner::GetColumnBool(cond_row.buf(), &row_view_, idxs_[0], row_view_.GetSchema()->Get(idxs_[0]).type());
}
const Row ProjectGenerator::Gen(const Row& row, const Row& parameter) {
    return CoreAPI::RowProject(fn_, row, parameter, false);
}

const Row ConstProjectGenerator::Gen(const Row& parameter) { return CoreAPI::RowConstProject(fn_, parameter, false); }
...
Row Runner::GroupbyProject(const int8_t* fn, const codec::Row& parameter, TableHandler* table) {
    ...
    auto udf = reinterpret_cast<int32_t (*)(const int64_t, const int8_t*, const int8_t*, const int8_t*, int8_t**)>(
        const_cast<int8_t*>(fn));
    int8_t* buf = nullptr;
...
const Row WindowProjectGenerator::Gen(const uint64_t key, const Row row, const codec::Row& parameter, bool is_instance,
                                      size_t append_slices, Window* window) {
    return Runner::WindowProject(fn_, key, row, parameter, is_instance, append_slices, window);
}
...
std::vector<std::shared_ptr<PartitionHandler>> WindowUnionGenerator::PartitionEach(
    ...
    if (!windows_gen_.empty()) {
        ...
        for (size_t i = 0; i < inputs_cnt_; i++) {
            union_partitions.push_back(windows_gen_[i].partition_gen_.Partition(union_inputs[i], parameter));
        }
    }
...
std::shared_ptr<TableHandler> IndexSeekGenerator::SegmnetOfConstKey(const Row& parameter,
...
std::shared_ptr<TableHandler> IndexSeekGenerator::SegmentOfKey(const Row& row, const Row& parameter,
                                                               std::shared_ptr<DataHandler> input) {
    auto fail_ptr = std::shared_ptr<TableHandler>();
    if (!input) {
        LOG(WARNING) << "fail to seek segment of key: input is empty";
...
std::shared_ptr<DataHandler> FilterGenerator::Filter(std::shared_ptr<PartitionHandler> partition,
                                                     const Row& parameter) {
    if (!partition) {
